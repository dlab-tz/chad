...
const getSpreadsheetData = (spreadsheetId, sheet, callback) => {
  authorize((auth) => {
    ...
    sheetsService.spreadsheets.values.get({
      ...
      return callback(err, result.data.values)
    })
  })
}
/**
 * @param {Array} values
 */
const addRow = (spreadsheetId, sheet, values) => {
  winston.info("Adding row into spreadsheet")
  authorize((auth)=> {
    let sheetsService = google.sheets({version: 'v4', auth})
    const resource = {
      values,
    ...
    sheetsService.spreadsheets.values.append({
      ...
      if (err) {
        ...
        winston.info(result.data.updates.updatedCells + ' cells added.')
      }
    });
  })
}
